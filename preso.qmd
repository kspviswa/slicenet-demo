...
epoch: 1
```
*_Delays_* slicelets (user traffic) as per `delay_pattern`, limited by `delay_pattern_threshold`
:::
::: {.column width="10%"}
:::
::: {.column width="30%"}
...
- default ^[no delays]
- exponential ^[delays added as per exponential distribution]
- uniform ^[delays added as per uniform distribution]
:::
::::
...
- random duration to be assigned ^[$n \in \{1, ..., duration-range\}$]
- load balancing slicelet allocation based on weightage ^[Eg for gold, `weight : 20` means `20%` of `N` slicelets will be assigned to `gold` service]
:::
::::
